...
function EmployeeLogin() {
  ...
  const [isAdminOrHr, setIsAdminOrHr] = useState(false); // State to track admin/HR token
  ...
  useEffect(() => {
    ...
    const accounts = tokenKeys.map((key) => {
      ...
      try {
        ...
        // Check if the token belongs to admin or HR
    ...
    // Check if the active token (if any) is for admin/HR
    ...
    if (activeToken) {
      try {
        ...
        // Optional: Auto-redirect to admin dashboard if admin/HR is active
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        {/* Display admin/HR status (optional) */}
        {isAdminOrHr && (
